...
while not wifi.is_connected:
    ...
    try:
        try:
            wifi.connect_AP(s.ssid1, s.password1)
        except:
            wifi.connect_AP(s.ssid2, s.password2)
...
while True:
    ...
    try:
        ...
        result = "date={:d}_{:02d}_{:02d};temp={:g};hum={:g}".format(
            t.tm_hour, t.tm_min, t.tm_sec, temperature, humidity)
        print(result)

        host = "http://192.168.1.108:5000/"
        url = host + result
        print(url)
        try:
            response_flask = requests.get(url)
        except:
            print("flask not responding !!!!")

        post_url = "https://" + s.TS_HTTP_SERVER + "/update"
        # Create payload
        payload = "api_key=" + s.TS_WRITE_API_KEY + "&field1=" + str(temperature) + "&field2=" + str(humidity)

        response = requests.post(post_url, data=payload)
        print("thingspeak status", response.status_code)
